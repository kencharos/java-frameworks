...
```
docker build -t helidon-se-sample .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 helidon-se-sample:latest
```
...
kubectl get service helidon-se-sample  # Get service info
```
...
```
docker build -t helidon-se-sample-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 helidon-se-sample-native:latest
```